...
export function NewsPage() {
      ...
      try {
        ...
        if (error) {
          ...
          if (interests && interests.length > 0) {
            ...
            setError('No interests found. Please add some interests on the interests page first.')
          }
        }
      } catch (err) {
        if (!isMounted) return
  ...
  const generateNews = async (interests: string[]) => {
    ...
    try {
      // Use EventSource for true streaming
      const eventSource = new EventSource('/api/news/stream?' + new URLSearchParams({
      ...
      eventSource.onmessage = (event) => {
        try {
          ...
          if (data.type === 'first-batch') {
            ...
            setIsLoading(false) // Set loading to false when streaming is complete
            eventSource.close()
               ...
               <div className="text-center py-4">
                 <div className="inline-flex items-center px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 rounded-lg">
                   ...
                   <span className="text-sm font-medium">Loading more articles...</span>
                           ...
                           {/* Loading indicator for new article requests - only show when not streaming */}
              {isLoading && articles.length > 0 && !isLoadingMore && (
                <div className="text-center py-4">
                  <div className="inline-flex items-center px-4 py-2 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 rounded-lg">
                    <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-green-600 mr-2"></div>
                    <span className="text-sm font-medium">Fetching new articles...</span>
                  </div>
                </div>
              )}
           </div>
